...
- **本轮未覆盖(如实记录)**:真机**完整启动**验证受阻于本地 dsh 检出缺 native binding——`dsh web` 启动即 `fatal: No usable native binding found for node-addon-require-builtin-darwin-arm64`(`node_modules/.pnpm/node-addon-require-builtin@0.1.6/.../build/` 无产物),与插件无关。替代证据:① 真实 dsh CLI `dsh --profile web --patch <本插件 cordis.patch.yml> --dump-config` 证明 patch **全部命中**(`ptc-runtime`/`workflow-ptc`/`tool-workflow` 三行被覆盖、锚点展开为完整表达式、insert 出 `ptc-cordis-runtime` 行)且**零 warn**;② 用与 loader 完全相同的求值器(`new Function('ctx','expr','with(ctx){return eval(expr)}')`)对合取逐项验证:无快照/开关关/`ready=false`/解释器被删/win32 等情形**都保持 Node 行**,满合取时才切 python,OFF 表达式是 ON 的严格取反;③ 冒烟 74/74。**待补**:真实启动下的 provider 切换与 run_code 呈现(python SDK)对照官方 `snapshots/session/ptc-python-turn/`。
