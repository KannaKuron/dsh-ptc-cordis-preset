...
- **生效态上报(纯加法,不改已发布字段语义)**:能力服务 `ptcCordisPreset` 增报 `pythonBackend: 'python' | 'node'`(取值 = 与 `!!js` **相同**的合取结论)与只读 `pythonIssue: string`(原因,供对面卡片显示);既有 `pythonRuntime` **保持 v0.15.0 的语义 = 用户意图**(gitbash v0.26.0 已在消费它,不可用时它仍为 true,不因后端不可用而变);`syncRuntimeSnapshot().applied` 语义精确化为「**本 boot 生效**」(刚写入的快照 → `false` + reason `takes effect on the next start`)。两侧通道分工:能力服务 = 生效态(gitbash 用它决定变体重建/workflow 互斥),settings 表单 = 用户意图。
- **验证**:`npm test` **79/79**(新增 5 条回归:代际判据、等价快照不重写、`pythonBin` 双面 schema、profile 口径预检、生效态字段)。真机(**隔离 DSH_HOME + Electron 宿主 + 探针**)三场景:① 冷启动即 ON(删快照)→ `failed=0`、`language=typescript`(本 boot 不生效)、host 半写出快照;② 热启动(快照已在)→ `failed=0`、`language=python`;③ 包从 profile `node_modules` 移走 → `failed=0`、`ready:false`、node 行保持。能力探针:可用时 `capability.pythonRuntime=true pythonBackend=python`;包缺失时旧语义实测为 `pythonRuntime=false pythonBackend=node` —— 按 Lead 裁决(不改已发布字段语义)修正后,`pythonRuntime` **保持用户意图 true**、生效态由 `pythonBackend=node` + `pythonIssue` 表达,该 intent/effect 分离由单测锁定,真机复核留给 task-10。
- **已知边界(写清,供集成阶段对齐)**:**settings 表单里的 `pythonRuntime` 是用户意图**(dsh 设置面所有物,插件不单方面回写用户配置);「不可用时实际没生效」由能力服务的 `pythonBackend`(与 `pythonIssue`)、卡片红字原因表达,而 `pythonRuntime` 始终是用户意图 ⇒ **gitbash 侧的 workflow 互斥应以 `pythonBackend` 为准(v0.26.1),读不到则回退旧行为**(读不到则回退旧行为),否则会出现「卡片显示已开启、实际跑 node 却白丢 workflow」。
...
- **真机验证(隔离 DSH_HOME + 打包版 Electron 作 node 宿主,开/关两态)**:自带 node 是 hardened runtime + 特定 Team ID,与该 adhoc 签名原生 addon 的 `dlopen` 冲突(`dsh web` 报 `fatal: No usable native binding found for node-addon-require-builtin-darwin-arm64`,与插件无关);改用 `ELECTRON_RUN_AS_NODE=1 "/Applications/DeepSeek Harness.app/Contents/MacOS/DeepSeek Harness" /opt/homebrew/bin/dsh --profile <隔离> --port <n> --no-open` 后**真实 web 启动跑通**(打印 `http://127.0.0.1:<n>/?token=…`),并加一个 `inject: ['ptcRuntime']` 探针行读真实 provider:**关态**(无快照)= `ptcRuntime.language=typescript isolation=process ctor=NodePtcRuntime`(官方 Node provider 原样)+ `preset 'ptc-cordis' registered declaratively (workflow ON)`;**开态**(快照 `{pythonRuntime:true,ready:true,pythonBin:/opt/homebrew/bin/python3}`)= 宿主日志 `experimental Python PTC runtime active (@deepseek-ai/dsh-experimental-ptc-runtime-python via loader resolver, /opt/homebrew/bin/python3) — run_code presents Python` 且探针 `ptcRuntime.language=python isolation=process ctor=PythonPtcRuntime`——**provider 真被替换且在 root realm 对 `tools` 行可见**。run_code 的语言 / SDK 提示词 / 工具呈现正是由这一个 `language` 驱动(`packages/core/tools/src/index.ts:40,1046`),故 python 侧呈现随之切换。**装配面对照(免凭据,证明「切到 python」= 官方 python 快照的呈现)**:run_code 的 schema/提示词文本由 `RUN_CODE_FLAVORS[runtime.language]` 投影(`packages/core/tools/src/ptc.ts:92-94,155-160`),两态 flavor 的 `codeDescription` 分别是 `The program: the body of an async TypeScript function.` 与 `The program: the body of an async Python function.`;官方快照里 `snapshots/session/ptc-python-turn/tool-schemas.expected.json` 正是前者之外的那句 **async Python function**,`snapshots/session/ptc-turn/tool-schemas.expected.json` 是 **async TypeScript function**,python 快照 `system-prompt.expected.md:34` 的 SDK 说明(`run_code` takes two required arguments: `code` — the body of an async Python function …)与 `system-prompt.expected.md:514` 的 `async def bash(self, args: BashArgs)` 亦与之同源——即真机上 `language` 的切换把呈现切到与官方 python 快照**同一份** flavor 文本。**比对范围**:flavor/schema 描述与 SDK 段为逐字比对;整份 system prompt 未逐字节比对(它还包含本插件自身的组合差异,逐字节对照无意义)。**探针未覆盖**:`tools.schemas()` 在 root scope 取不到 `run_code`(该工具注册在 preset/agent scope),故 schema 断言以「language → flavor 表 → 官方快照同字符串」三段链完成,未在真机内直接 dump schema 文本。
